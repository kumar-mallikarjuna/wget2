...
int main(void)
{
	...
	wget_snprintf(options, sizeof(options),
		"--iri -rH -e http_proxy=localhost:%d -e https_proxy=localhost:%d --local-encoding=EUC-JP " euc_jp_hostname,
...
// UTF-8 command line characters are mangled on MinGW on C locale
	...
	// test-idn-cmd
	wget_snprintf(options, sizeof(options),
		"--iri -rH -e http_proxy=localhost:%d -e https_proxy=localhost:%d --local-encoding=UTF-8 " utf8_hostname,
		wget_test_get_http_server_port(),
		wget_test_get_h2_server_port());
